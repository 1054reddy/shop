...
def checkout():
    ...
    try:
        ...
        # Step 3: Update stock for each product size with a check
        cursor.execute("""
        ...
        # Check if stock update affected all items; otherwise, rollback
        if cursor.rowcount < cart_item_count:
            raise mysql.connector.Error("Insufficient stock for one or more items.")
